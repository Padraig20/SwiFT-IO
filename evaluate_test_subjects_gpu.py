...
def evaluate_subjects(run_id='opr6oq97'):
    ...
    # Setup data module - GPU optimized version
    args_model_dict["num_workers"] = 2
    args_model_dict["eval_num_workers"] = 2
    ...
    args_model_dict['eval_batch_size'] = 4
    ...
    model.eval()
    model.to(device)
    print("✅ Model & Data initialized (GPU-accelerated)")
    ...
    for subj_idx, subject in enumerate(test_subjects, 1):
        ...
        # Create dataloader - GPU optimized
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(test_loader):
                ...
                fmri_seq = data['fmri_sequence'].float().to(device)
                target = data['target'].float().to(device)  # Shape: [batch, time, emotions]
                ...
                all_targets.append(target_mean.cpu().numpy())
