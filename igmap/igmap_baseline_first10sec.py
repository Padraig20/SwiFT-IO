#!/usr/bin/env python3
"""
IG Map with baseline = first 10 seconds average (새로운 방식)

Model: opr6oq97
Baseline: 영화 초반 10초(10 TRs)의 평균 뇌 활성화
"""
...
def load_first_n_frames(subject_path, n_frames=10):
    """
    Load first n frames from a subject and return average

    Args:
        subject_path: Path to subject directory (e.g., .../img/sub-XXXX)
        n_frames: Number of initial frames to average

    Returns:
        Average of first n frames [1, X, Y, Z, 1]
    """
    frames = []
    for i in range(n_frames):
        frame_path = Path(subject_path) / f"frame_{i}.pt"
        if frame_path.exists():
            frame = torch.load(frame_path, weights_only=True)
            frames.append(frame)
        else:
            print(f"⚠ Warning: {frame_path} not found", flush=True)

    if not frames:
        raise FileNotFoundError(f"No frames found in {subject_path}")

    # Stack and average: [N, X, Y, Z, 1] -> [X, Y, Z, 1]
    frames_tensor = torch.stack(frames, dim=0)
    avg_frame = frames_tensor.mean(dim=0, keepdim=False)  # [X, Y, Z, 1]
...
def process_subject(args_tuple):
    ...
    # ========== BASELINE: FIRST 10 SECONDS AVERAGE ==========
    # Load first 10 frames and compute average
    image_path = "/scratch/HBN/3.3.1.movieDM_MNI_to_TRs_smooth_znorm_241120/img"
    subject_path = Path(image_path) / subject

    print(f"  [Baseline] Loading first 10 frames from {subject_path}...", flush=True)
    avg_first_10 = load_first_n_frames(subject_path, n_frames=10)  # [X, Y, Z, 1]

    # Need to match input shape: [B, C, X, Y, Z, T]
    # avg_first_10 is [X, Y, Z, 1], we need to expand to match sequence length
    ...
    input_shape = first_data['fmri_sequence'].shape  # [B, C, X, Y, Z, T]

    # Expand avg_first_10 to match time dimension
    # [X, Y, Z, 1] -> [1, 1, X, Y, Z, T] by repeating
    baseline_template = avg_first_10.unsqueeze(0).unsqueeze(0)  # [1, 1, X, Y, Z, 1]
    baseline_template = baseline_template.repeat(1, 1, 1, 1, 1, input_shape[-1])  # [1, 1, X, Y, Z, T]

    print(f"  [Baseline] first 10sec avg, shape: {baseline_template.shape}", flush=True)
    print(f"  [Baseline] value range: [{baseline_template.min():.4f}, {baseline_template.max():.4f}]", flush=True)
    # ========================================================
    ...
    for data in test_loader:
        ...
        # Use the prepared baseline
        baseline = baseline_template.clone()

        for i, emotion_label in enumerate(emotion_labels):
            out_dir = project_root / f"analysis/4_IGmap/baseline_first10sec/{args.run_id}/nii_segments" / subject / f"target{i}_{emotion_label}"
