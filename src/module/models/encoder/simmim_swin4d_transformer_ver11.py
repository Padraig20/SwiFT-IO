...
class Simmim_RoPE4DSwinTransformer(nn.Module):
    """
    ...
    def forward_encoder(self, x: torch.Tensor, mask: torch.Tensor): 
        # patch embedding
       ...
       # x = self.pos_drop(x)  # B C D H W T 
        ...
        if self.decoder_type == 'linear':
            x = x.flatten(2).permute(0, 2, 1)   # B L C
            x = self.norm(x)    # B L C
        elif self.decoder_type == 'w/ape':  # 250219 jubin. in the frontier code, it returns just x
            b, c, d, h, w, t = x.shape 
            x = x.flatten(2).permute(0, 2, 1)  # B L C 
            x = self.norm(x)
            x = x.permute(0, 2, 1).reshape(b, c, d, h, w, t)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor):
        """
        x: B, 1, D, H, W, T 
        mask: B, D//p, H//p, W//p, T//p
        """
        orig_img_shape = x.shape 
        orig_mask_shape = mask.shape  
        # Input x for encoder is expected as (B, C, D, H, W, T_in_original_img_size)
        x = x.permute(0, 1, 5, 2, 3, 4).contiguous() # B, C, T_img, D_img, H_img, W_img
        mask = mask.permute(0, 4, 1, 2, 3).contiguous() # B, T//p, D//p, H//p, W//p

        if self.to_float:
            x = x.float()
        z = self.forward_encoder(x, mask)   # b, c, d, h, w, t
        x_rec = self.forward_decoder(z) # B, 1, D, H, W, T
        mask = mask.repeat_interleave(self.patch_size[0], 1).repeat_interleave(self.patch_size[1], 2).repeat_interleave(self.patch_size[2], 3).repeat_interleave(self.patch_size[3], 4).unsqueeze(1)   # B, C, T_img, D_img, H_img, W_img 
        mask = mask.permute(0, 1, 3, 4, 5, 2).contiguous() 
